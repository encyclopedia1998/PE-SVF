...
def write_PE_SVF_table(SVF_actual_dict, SVF_desired_dict, out_PE_SVF_table):
    ...
    for FID in SVF_actual_dict:
        ...
        PE_SVF = AOP_SVF-DOP_SVF
        w_str_list.append(",".join([str(DOPid), str(DOP_SVF), 
            str(AOPid), str(AOP_SVF), str(PE_SVF)]))

    with open(out_PE_SVF_table, "w") as w:
        w.write("DOPid,DOP_SVF(10^-2),AOPid,AOP_SVF(10^-2),PE_SVF(10^-2)\n")
        w.write("\n".join(w_str_list))
...
def main():
    ...
    if not path.exists(out_path):
        makedirs(out_path)
    out_PE_SVF_table = "/".join([out_path, "PESVF.csv"])
    ...
    write_PE_SVF_table(SVF_actual_dict, SVF_desired_dict, out_PE_SVF_table)
